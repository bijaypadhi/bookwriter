...
function selectTemplate() {
	const templates = [
        { id: 'template1', name: 'Template 1', img:'template1.jpg'},
        { id: 'template2', name: 'Template 2', img:'src/images/template1.jpg'},
        { id: 'template3', name: 'Template 3', img:'src/images/template1.jpg'}
    ];
...
function applyTemplate(templateId) {
    textArea = null;  // Reset textArea
    const imgElement = document.getElementById("imgPageLeft");
    const imgPageElement = document.getElementById("imgPageRight");
    
    // Extract the file name from imgElement.src
    const imgSrc = imgElement.src;
    const fileName = imgSrc.substring(imgSrc.lastIndexOf('/') + 1);

    // Extract the number from the file name (assuming it's something like "1.jpg", "2.jpg", etc.)
    ...
    if (fileNumber % 2 === 1) { // Odd number
        ...
        imgPageElement.onload = function() {
            ...
            imgElement.style.backgroundColor = rgbaColor;
            ...
            const jpegDataUrl = colorCanvas.toDataURL('image/jpeg');
            imgElement.src = jpegDataUrl;

            container = document.createElement('div');
            if (container.classList.contains('text-area-container')) {
                container.classList.remove('text-area-container');
                } else {
                container.classList.add('text-area-container');
               }
            container.style.position = 'absolute';
            container.style.left = `${imgElement.offsetLeft}px`;
            container.style.top = `${imgElement.offsetTop}px`;
            container.style.width = `${imgElement.clientWidth}px`;
            container.style.height = `${imgElement.clientHeight}px`;

            // Only create the text area if it hasn't been created yet and file number is even
            if (textArea == null) {    
                textArea = createTextArea(imgElement);
                container.appendChild(textArea);

                const micButton = document.createElement('i');
                micButton.classList.add('fas', 'fa-microphone-slash', 'mic-button');
                micButton.style.position = 'absolute';
                micButton.style.right = '10px';
                micButton.style.top = '100px';
                micButton.style.fontSize = '24px';
                micButton.style.color = 'black';
                micButton.onclick = () => toggleSpeechRecognition(micButton, textArea);
...
function createTextArea(imgPageElement) {
...
function saveCanvases() {
    // Display alerts for debugging purposes
   
    if (canvas && colorCanvas) {
        // Ensure PAGE is defined before using it
        if (typeof PAGE === 'undefined') {
            alert('PAGE is not defined');
            return; // Exit the function if PAGE is not defined
        }

        const pageNo = PAGE + 1; // Calculate the next page number

        // Save the canvases with the respective file names
        saveCanvasAsJpg(canvas, `${PAGE}.jpg`);
        saveCanvasAsJpg(colorCanvas, `${pageNo}.jpg`);

        // Optionally, increment PAGE for the next usage if it's a global variable
    
    } else {
        alert('Canvas or colorCanvas is not initialized'); // Error handling if canvases are missing
    }
}
...
document.getElementById('saveButton').addEventListener('click', saveCanvases);
